...
from .collectors import collect_gdelt, collect_openalex, write_records
...
def collect(config: Any, as_of: date) -> Path:
    ...
    academic = collect_openalex(config, as_of)
    corporate = collect_gdelt(config, as_of)
    outputs = []
    for source, records in (("openalex", academic), ("gdelt", corporate)):
        output = config.paths["interim_dir"] / f"activity_records_00_{as_of.isoformat()}_{source}.jsonl"
        write_records(records, output)
        outputs.append(str(output))
    failed_corporate = sum(record.get("collection_status") == "failed" for record in corporate)
    print(json.dumps({"outputs": outputs, "academic_records": len(academic), "corporate_records": len(corporate), "corporate_failed": failed_corporate}, ensure_ascii=False, indent=2))
